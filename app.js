...
// Uso: referencie este arquivo após index.html (defer ok)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const btnBack = document.getElementById('btnBack'); // optional in layout
  ...
  const openScansList = document.getElementById('scansList');
  ...
  function renderScans() {
    ...
    scanRecords.forEach((r, i) => {
      ...
      div.innerHTML = `<div style="display:flex;gap:8px;align-items:center">
        ...
        <div>
          <button data-i="${i}" class="edit-btn" style="background:#00b4d8;color:white;padding:6px;border-radius:6px">Editar</button>
        </div>
      </div>`;
  ...
  function cmToPx(cm) {
    // Convert cm to px based on typical 96dpi: 1in = 2.54cm, 96px/in
    return Math.round((96 / 2.54) * cm);
  }
  ...
  function fitCanvases() {
    try {
      ...
      // draw the 10cm box centered (10cm ~ cmToPx(10))
  ...
  function drawBoundingBox(loc) {
    ...
    // desired 10cm square (in px)
    const boxPx = Math.min(Math.round(Math.min(w, h) * 0.6), cmToPx(10));
    const boxW = boxPx, boxH = boxPx;
    ...
    // if loc supplied (from jsQR), draw green polygon
    if (loc) {
  ...
  /* ---------- outros eventos UI ---------- */
  btnCamera.addEventListener('click', () => { if (!currentUser) return; if (contentArea) contentArea.style.display = 'none'; if (cameraContainer) cameraContainer.style.display = 'flex'; startScanner(); });
  ...
  if (btnBack) btnBack.addEventListener('click', () => { stopScanner(); if (cameraContainer) cameraContainer.style.display = 'none'; showDeliveries(); });
  ...
  if (sidebar) sidebar.style.display = 'none'; // hide until login
