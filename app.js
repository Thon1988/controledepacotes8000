...
(() => {
    ...
    function renderScans() {
        ...
        scans.forEach(item => {
            ...
            const idType = 'Link Completo'; // Já que estamos priorizando o link
            ...
            // Alteração #2: Apenas endereço e CEP para o mapa
            const addressOnly = `${item.comprador?.endereco || 'N/A'}, CEP ${item.comprador?.cep || 'N/A'}`.replace(/, CEP N\/A$/, '').trim();
            const fullAddress = `${item.comprador?.nome || 'N/A'}, ${addressOnly}`.trim();
    ...
    async function handleScanResult(payload) {
        ...
        // Alteração #1: Priorizar o link completo como ID principal
        const mainId = payload; // Usa o payload bruto como ID principal
        ...
        showPopup(`OK • ${mainId}`); // Exibe o mainId (Link_Completo) no popup
